...
import {
  and, or, eq, asc, desc, gt, lt, AnyColumn, SQL,
} from 'drizzle-orm';
...
>({
  ...
  const singleColumnPaginationWhere = typeof primaryCursor !== 'undefined'
    ? primaryOperator(primaryColumn, primaryCursor)
    : undefined;

  // Double cursor pagination
  const doubleColumnPaginationWhere = secondaryColumn
    && secondaryOperator
